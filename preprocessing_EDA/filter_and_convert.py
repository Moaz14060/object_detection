...
# CONFIG
# =============================
BASE_DIR = r"E:\all e drive\AIU COMPUTER SCIENCE BACLERIOS\AIU COMPUTER SCIENCE SIXTH SEMESTER SPRING (24-25)\DEPI\Final Project\CocoDataSet\coco_preprocess\Milestone_1_Data_Preprocessing\outputs"
DATASET = os.path.join(BASE_DIR, "filtered_dataset")
OUT_DIR = os.path.join(BASE_DIR, "balanced_dataset")
IMG_SIZE = 640
RARE_THRESHOLD = 0.05  # Classes with less than 5% of total instances are considered rare
...
rare_classes = [cls for cls, cnt in class_counts.items() if cnt / total_instances < RARE_THRESHOLD]
print("Detected rare classes:", rare_classes)

# =============================
# STEP 2: Define augmentations
# =============================
def augment_image(img):
    """Apply light random augmentation"""
    # Horizontal flip
...
# =============================
# STEP 3: Process splits and augment
# =============================
for split in splits:
    ...
    out_label_dir = os.path.join(OUT_DIR, split, "labels")

    for fname in tqdm(os.listdir(img_dir)):
        if not fname.endswith((".jpg", ".jpeg", ".png")):
            continue
        src_img_path = os.path.join(img_dir, fname)
        src_label_path = os.path.join(label_dir, os.path.splitext(fname)[0] + ".txt")

        if not os.path.exists(src_label_path):
            continue

        # Load labels and check for rare classes
        with open(src_label_path, "r") as f:
            lines = f.readlines()
        img_classes = [int(line.split()[0]) for line in lines]
        has_rare = any(c in rare_classes for c in img_classes)

        # Copy original image and label
        img = Image.open(src_img_path).convert("RGB")
        img_resized = img.resize((IMG_SIZE, IMG_SIZE))
        img_resized.save(os.path.join(out_img_dir, fname), "JPEG", quality=80)
        shutil.copy(src_label_path, os.path.join(out_label_dir, os.path.splitext(fname)[0] + ".txt"))

        # Augment only if contains rare class
        if has_rare:
            img_aug = augment_image(img_resized)
            aug_name = os.path.splitext(fname)[0] + "_aug.jpg"
            img_aug.save(os.path.join(out_img_dir, aug_name), "JPEG", quality=80)
            shutil.copy(src_label_path, os.path.join(out_label_dir, os.path.splitext(aug_name)[0] + ".txt"))

# =============================
# STEP 4: Create data_balanced.yaml
# =============================
...
data_yaml = {
    ...
    "val": os.path.abspath(os.path.join(OUT_DIR, "val", "images")),
    "nc": len(CLASS_NAMES),
    "names": CLASS_NAMES
}
...
print("\n✅ Balanced dataset created at:", OUT_DIR)
print("✅ YOLO YAML file created at:", yaml_path)
